...
template <typename etype, typename lambda, typename std :: enable_if <std :: is_same <etype, events :: molecule> :: value> :: type *> size_t engine :: on(const uint8_t & tag, const lambda & callback)
{
  :: callback <events :: molecule> * wrapper = new :: callback <events :: molecule, lambda> (callback);
